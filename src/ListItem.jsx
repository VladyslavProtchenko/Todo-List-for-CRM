...
export default function ListItem({data, id, brands, onFocus, setBrands, remove, setItemStatus, chooseItem,activeBrands,setActiveBrands}) {
    ...
    useEffect(()=>{
        brands.map(item=>{
            if(item.id === activeBrandLabel.id) {
                item.isActive = true      
            }
        })
        setBrands(brands)
  ...
  return (
    ...
    <div className={`${container} ${readyToDelete && !data.isChosen &&'opacity-30'} ${data.isChosen && 'bg-zinc-500 hover:bg-zinc-500'}` } onClick={()=>chooseItem(id)}>
        ...
        <div 
            className={` ${enable?switchContainerON:switchContainerOF}`} 
            onClick={(e)=> {
                e.stopPropagation()
                setEnable(!enable)
        ...
        <div className={ItemName} >
            ...
            <div className={BrandLabel} onClick={(e)=>{
                e.stopPropagation()
        ...
        <div 
            ...
            onClick={()=>remove(id)}
            onMouseEnter={()=>setReadyToDelete(true)}  onMouseLeave={()=>setReadyToDelete(false)}
        >
